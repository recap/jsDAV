...
var jsDAV_Handler = module.exports = function(server, req, resp) {
    ...
    try {
		function forbid(msg) {
			...
			var headers = [];
			self.httpResponse.writeHead(403, headers);
			self.httpResponse.end();
		}

		if (jsDAV.enforceTokenAuth) {
			var token = this.httpRequest.headers.authorization.replace('Bearer ','');
			if (!token) {
				forbid('No token');
				return;
			}
			...
			var cert = jsDAV.superUsers[user].decodedPublicKey
			if (!user || !jsDAV.superUsers || !jsDAV.superUsers[user] || !cert) {
				forbid('No user');
				return;
			}
...
(function() {
    ...
    this.httpPropfind = function() {
		...
		console.log('HEADERS: ' + JSON.stringify(this.httpRequest.headers, null, 2))
